...
def feature_gen_pipeline(df):
    ...
    for col in tqdm(numeric_columns):
        try:
            ...
            agg[f"{col}_ma3_r1_r2"] = agg[f"{col}_ma3_r1"] / agg[f"{col}_ma3_r2"].replace(0, 0.001)
            drop_columns.append(f"{col}_ma2_r2")
            drop_columns.append(f"{col}_ma2_r3")
            drop_columns.append(f"{col}_ma3_r2")
            ...
            drop_columns.append(f"{col}_second_last")
            drop_columns.append(f"{col}_third_last")
            agg[f"{col}_acceleration"] = agg[f"{col}_sprint"] / agg[f"{col}_previous_sprint"].replace(0, 0.001)
            gc.collect()
            ...
            agg[f"{col}_velocity"] = agg[f"{col}_displacement"] / np.log(agg["num_statements"] + 1)
            agg[f"{col}_last_minus_avg"] = agg[f"{col}_last"] - agg[f"{col}_avg"]
            agg[f"{col}_last_minus_midpoint"] = agg[f"{col}_last"] - 0.5 * (agg[f"{col}_max"] + agg[f"{col}_min"])
    ...
    drop_columns.extend(get_cols(agg, "temp"))
...
# Clipping logic
def clip_all(agg, max_thr=1e4, min_thr=-1e4):
    # Clip upper bound
    agg_df = agg.select_dtypes(np.number)
    max_ = agg_df.max()
