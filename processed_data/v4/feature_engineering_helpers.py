...
def feature_gen_pipeline(df):
    ...
    for col in tqdm(numeric_columns):
        try:
            ...
            agg[f"{col}_general_trend"] = 10 * (agg[f"{col}_ma2_r1"] - agg[f"{col}_ma2_first"]) / agg["num_statements"]
            drop_columns.append(f"{col}_ma2_first")
            gc.collect()
            ...
            agg[f"{col}_distortion"] = agg[f"{col}_skew"] / (agg[f"{col}_std"].replace(0, 1e-3))
            agg[f"{col}_last_minus_avg"] = agg[f"{col}_last"] - agg[f"{col}_avg"]
            agg[f"{col}_last_minus_ma2_r2"] = agg[f"{col}_last"] - agg[f"{col}_ma2_r2"]
            agg[f"{col}_coef_var"] = (agg[f"{col}_std"] / agg[f"{col}_avg"]).replace(
                [np.inf, -np.inf], np.nan
            )
            gc.collect()
        except:
